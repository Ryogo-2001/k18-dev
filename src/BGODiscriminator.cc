...
bool BGODiscriminator::SelectRange(double threshold,double begin,double last)
{
  ...
  bool LorS_flag = false; 
  if (threshold > 0) {
    LorS_flag = true;     
  }
  GetCrossPoint(threshold, LorS_flag);
  return true;
...
int BGODiscriminator::GetCrossPoint(double threshold, bool LorS)
{
  //int FoundNum = GetPeakNum(threshold,false,false);
  int FoundNum = GetPeakNum(threshold, false, LorS);
  if(FoundNum>0){
