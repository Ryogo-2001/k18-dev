...
std::vector<Double_t> km_mom_x;
  ...
  double G4p1vtx_x;//Production vertex, identical to l decay vert
  double G4p1vtx_y;
  double G4p1vtx_z;
  double G4p1mom;
  double G4p1mom_x;
  ...
  double G4p2vtx_x;//Production vertex, identical to l decay vert
  double G4p2vtx_y;
  double G4p2vtx_z;
  double G4p2mom;
  double G4p2mom_x;
...
//G4 Initialization
    ...
    G4p1id = qnan, G4p1tid = qnan, G4p1nh = 0, G4p1tnh = 0;
    G4p1vtx_x = qnan, G4p1vtx_y = qnan, G4p1vtx_z = qnan;
    G4p1mom = qnan, G4p1mom_x = qnan, G4p1mom_y = qnan, G4p1mom_z = qnan;

    p1tid = qnan, p1nh = 0;
    p1vtx_x = qnan, p1vtx_y = qnan, p1vtx_z = qnan;
    p1mom = qnan, p1mom_x = qnan, p1mom_y = qnan, p1mom_z = qnan;

    G4p2id = qnan, G4p2tid = qnan, G4p2nh = 0, G4p2tnh = 0;
    G4p2vtx_x = qnan, G4p2vtx_y = qnan, G4p2vtx_z = qnan;
    G4p2mom = qnan, G4p2mom_x = qnan, G4p2mom_y = qnan, G4p2mom_z = qnan;

    p2tid = qnan, p2nh = 0;
    p2vtx_x = qnan, p2vtx_y = qnan, p2vtx_z = qnan;
    p2mom = qnan, p2mom_x = qnan, p2mom_y = qnan, p2mom_z = qnan;
...
dst::DstRead( Int_t ievent )
{
  ...
  TLorentzVector G4LvXi(event.G4ximom_x,event.G4ximom_y,event.G4ximom_z,hypot(event.G4ximom,XiMass));
  TLorentzVector G4LvP(event.G4p1mom_x,event.G4p1mom_y,event.G4p1mom_z,hypot(event.G4p1mom,ProtonMass));
  TLorentzVector G4LvLL = G4LvXi + G4LvP;
...
*/
  ...
  //if extra p, pi- pair exist, it's not LL events
	...
	*/
	...
	if(prev_LLfit_chisqr > chisqr_KFkk_ll_vertex){
	  ...
	  event.KFdecays_mom[0] = KFTVP1.Mag();
	  event.KFdecays_mom_x[0] = KFTVP1.x();
	  event.KFdecays_mom_y[0] = KFTVP1.y();
	  event.KFdecays_mom_z[0] = KFTVP1.z();
	  event.KFdecays_mom[1] = KFTVPi1.Mag();
	  event.KFdecays_mom_x[1] = KFTVPi1.x();
	  event.KFdecays_mom_y[1] = KFTVPi1.y();
	  event.KFdecays_mom_z[1] = KFTVPi1.z();
	  event.KFdecays_mom[2] = KFTVP2.Mag();
	  event.KFdecays_mom_x[2] = KFTVP2.x();
	  event.KFdecays_mom_y[2] = KFTVP2.y();
	  event.KFdecays_mom_z[2] = KFTVP2.z();
	  event.KFdecays_mom[3] = KFTVPi2.Mag();
	  event.KFdecays_mom_x[3] = KFTVPi2.x();
	  event.KFdecays_mom_y[3] = KFTVPi2.y();
	  event.KFdecays_mom_z[3] = KFTVPi2.z();
  ...
  if(event.xiflag){
    ...
    if(event.residual_multi==1 && event.p_multi==1){
      ...
      event.p1mom = event.residual_mom[0];
      event.p1mom_x = event.residual_mom_x[0];
      event.p1mom_y = event.residual_mom_y[0];
      event.p1mom_z = event.residual_mom_z[0];

      int id_p1 = event.residual_id[0];
    ...
    int id_p2 = GFxi_p_id_container.at(best_xi);
...
#if XiRecon
    if(G4TrackID.at(id_p1) == event.G4pid &&
       G4TrackID.at(id_pi1) == event.G4pi1id){
      event.lgood = true;
    ...
    event.p2mom = xi_p_mom_container[best_xi].Mag();
    event.p2mom_x = xi_p_mom_container[best_xi].x();
    event.p2mom_y = xi_p_mom_container[best_xi].y();
    event.p2mom_z = xi_p_mom_container[best_xi].z();
